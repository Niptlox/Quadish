...
# --- Новые предметы и растения: пиксель-арт сетками ---------------------
# Рисуем тем же способом, что и существ (units/Graphics/PixelArt.py), а не
# файлами: сетку видно в коде, её можно поправить не выходя из редактора, и она
# не зависит от того, найдётся ли png в сборке. Обозначения те же — B базовый,
# D тень, L блик, K обводка, w белый, y жёлтый, g зелёный, b голубой, o
# оранжевый, r красный, v фиолетовый, s камень, '.' прозрачный.
from units.Graphics.PixelArt import build_sprite as _grid


def _item_img(rows, base, scale=2):
    """Спрайт предмета из сетки. Масштаб целый — пиксели остаются квадратными."""
    return _grid(rows, base=base, size=(len(rows[0]) * scale, len(rows) * scale))


def _growth_frames(rows, base, scale=4):
    """Три кадра роста одного растения: росток, полурост, зрелое.

    Кадры выводятся из ОДНОЙ сетки гашением верхних рядов, а не рисуются
    отдельно: у ростка и зрелого растения обязан совпадать стебель, иначе при
    смене кадра растение дёргается. Заодно новое растение — это по-прежнему
    одна сетка, а не три.
    """
    frames = []
    for cut in (5, 3, 0):
        grid = ["." * len(r) for r in rows[:cut]] + list(rows[cut:])
        frames.append(_item_img(grid, base, scale))
    return frames


# Ведро: пустое и с водой. Разница ровно одна — залитая синим внутренность,
# потому что игрок должен различать их в тулбаре одним взглядом.
_BUCKET_EMPTY = [
    "K.......K",
    "KK.....KK",
    ".KKKKKKK.",
...
bucket_item_img = _item_img(_BUCKET_EMPTY, "#78716C")

# Удочка: длинное удилище по диагонали и леска с поплавком. Читается как
# инструмент, а не как палка, именно из-за поплавка.
_ROD = [
    ".......KB",
    "......KBB",
    ".....KBB.",
...
rod_item_img = _item_img(_ROD, "#A16207")

# Продукты хозяйства. Молоко и яйцо — еда, пух — материал.
_MILK = [
    ".KKKKK.",
    ".KwwwK.",
    "KwwwwwK",
...
milk_item_img = _item_img(_MILK, "#F5F5F4")
...
egg_item_img = _item_img(_EGG, "#FDE68A")
...
down_item_img = _item_img(_DOWN, "#E7E5E4")

# Кормушка: корыто с сеном. Блок, а не предмет, поэтому сетка на всю клетку.
_TROUGH = [
    "........",
    "..gg.g..",
    ".ggggg..",
...
trough_img = _item_img(_TROUGH, "#A16207", scale=4)
bucket_water_item_img = _item_img(_BUCKET_WATER, "#78716C")

# Таблетка: горсть, а не одна пилюля — она и крафтится пачкой.
_PILL = [
    "..KKK....",
    ".KBBBK...",
    ".KBwBK...",
...
pill_item_img = _item_img(_PILL, "#84CC16")

# Зелья: одна форма склянки на все, разный цвет содержимого. Так они читаются
# как один класс предметов, а не как шесть разных вещей.
_FLASK = [
    "..KKK..",
    "..KsK..",
    ".KKKKK.",
...
potion_speed_item_img = _item_img(_FLASK, "#38BDF8")
potion_strength_item_img = _item_img(_FLASK, "#F97316")
potion_stone_item_img = _item_img(_FLASK, "#A8A29E")
potion_fire_item_img = _item_img(_FLASK, "#FB7185")
potion_breath_item_img = _item_img(_FLASK, "#22D3EE")

# Трофеи существ. Каждый — узнаваемый силуэт: игрок должен понимать, с кого
# это упало, не читая название.
_FANG = [
    "..www..",
    ".wwwww.",
    ".KwwwK.",
...
fang_item_img = _item_img(_FANG, "#E7E5E4")
...
feather_item_img = _item_img(_FEATHER, "#A8A29E")
...
wing_item_img = _item_img(_WING, "#3F3A36")
...
sting_item_img = _item_img(_STING, "#C084FC")
...
core_item_img = _item_img(_CORE, "#78716C")
...
scale_item_img = _item_img(_SCALE, "#1E3A5F")

# Растения. Кроме камыша все растут пучком в клетке, поэтому сетка узкая и
# высокая: на 32 пикселях иначе не читается ни один стебель.
_REED = [
    "..g...g.",
    ".gg.gg..",
    "..g.g.g.",
...
reed_imgs = _growth_frames(_REED, "#CA8A04")
reed_img = reed_imgs[-1]     # зрелое растение — оно же иконка предмета
...
firebloom_imgs = _growth_frames(_FIREBLOOM, "#EA580C")
firebloom_img = firebloom_imgs[-1]     # зрелое растение — оно же иконка предмета
...
moonbloom_imgs = _growth_frames(_MOONBLOOM, "#C7D2FE")
moonbloom_img = moonbloom_imgs[-1]     # зрелое растение — оно же иконка предмета
...
cave_mushroom_img = _item_img(_CAVE_MUSHROOM, "#F87171", scale=4)
...
blore_moss_img = _item_img(_BLORE_MOSS, "#2563EB", scale=4)
...
tile_imgs = {None: none_img,
             ...
             427: egg_item_img,
             428: down_item_img,
             411: bucket_water_item_img,
             412: pill_item_img,
             418: _item_img(_PILL, "#22C55E"),
             413: potion_speed_item_img,
             414: potion_strength_item_img,
             415: potion_stone_item_img,
             416: potion_fire_item_img,
             417: potion_breath_item_img,
             420: fang_item_img,
             421: feather_item_img,
             422: wing_item_img,
             423: sting_item_img,
             424: core_item_img,
             425: scale_item_img,
...
IDX_TOOLS = {501, 502, 503, 530, 531, 532, 533, 581, 610,
             611, 612, 613, 614, 615,
             # Ведра (units/Tools/ToolBucket.py): предмет с собственным
             # действием, а не блок — набрать воду и вылить её
             410, 411,
             # Удочка (units/Tools/ToolFishing.py)
             520}
...
# Растения на дёрне: к траве и кусту добавились камыш, огнецвет и лунный цвет
ON_EARTHEN_PLANTS = {101, 102, 103, 104, 107, 108, 109}
# Растения на камне — под землёй. У них своя ветка в генераторе
# (GameMap.generate_chunk_noise_island): дёрна на глубине нет.
CAVE_PLANTS = {111, 112}
...
SEMIPHYSBODY_TILES = {106, 120, 127, 126, 125, 121, 129, 131, 122, 104, 300, 140, 224, 226, 227,
                      113,   # кормушка: через неё проходят, как через стол
                      # Новые растения: сквозь них проходят, как сквозь траву
                      107, 108, 109, 111, 112,
...
STANDING_TILES = {0, 110, 120, 121, 122, 123, 125, 126, 130, 129, 131, 113} | ON_EARTHEN_PLANTS | CAVE_PLANTS
# Задние панельки
BACKTILES = {1003, }
...
ITEM_TILES = {None, 51, 52, 53, 55, 56, 58, 61, 62, 63, 64, 65, 66, 301, 351,
             ...
             # Зелья, ведра, таблетки и трофеи существ — предметы, а не блоки
...
CLASS_TILE = {131, 129, 200, 210, 211, 212, 213, 214, 215, 216, 217, 218, 219, 220, 221, 222, 223,
              ...
              # Котёл — машина (топливо + вода + ингредиент), а не мебель
              125,
              # Кормушка (домашнее хозяйство)
              113,
...
CLASS_UPDATING_TILES = {131, 200, 210, 211, 212, 214, 215, 216, 217, 218, 219, 220, 221, 222, 223,
                        ...
                        # Котёл варит по своему таймеру — как печка
                        125,
                        # Кормушка кормит скот по таймеру
                        113}
...
# Растут по таймеру. Камыш, огнецвет и лунный цвет добавлены сюда не ради
# красоты: без роста посаженное растение стоит вечно, и грядка на базе
# ничем не отличается от декорации.
PLANT_WITH_TIMER = {101, 102, 107, 108, 109}
# Растения с тремя стадиями роста: собирать можно только зрелое.
GROWING_PLANTS = {107, 108, 109}
GROWING_PLANT_STAGES = 3
# растетет только на земле
PLANT_STAND_ON_DIRT = {101, 102, 103, 104, 251, 107, 108, 109}
...
Eats = {52: 10, 53: 2, 56: 8, 55: 100, 401: 7, 251: 7, 351: 1,
        ...
        # Таблетка лечит НЕ мгновенно: её работа — эффект заживления
        # (units/Effects.py ITEM_EFFECTS), поэтому сразу она даёт немного.
        412: 4,
        418: 8,
        # Продукты хозяйства: молоко и яйцо — обычная еда, но своя, с базы
        426: 12, 427: 6,
        # Зелья с длящимися эффектами. Мгновенного лечения не дают вообще:
        # у них другая роль, и «выпил — восполнил здоровье» размывало бы её.
...
biomes_plants_chance = {
    ...
    0: {101: 0.1, 103: 0.2, 104: 0.2, 108: 0.05, None: 0.45},   # desert
    1: {104: 0.6, 101: 0.05, 102: 0.03, 107: 0.04, None: 0.28},  # savanna
    2: {101: 0.25, 102: 0.15, 104: 0.5, 251: 0.02, None: 0.08},  # tropical_woodland
    3: {101: 0.3, 102: 0.1, 109: 0.08, None: 0.42},             # tundra
    4: {102: 0.3, 101: 0.15, 104: 0.4, None: 0.15},            # seasonal_forest
    5: {102: 0.35, 101: 0.25, 104: 0.28, 251: 0.03, 107: 0.05, None: 0.04},  # rainforest
    6: {102: 0.25, 101: 0.2, 104: 0.45, None: 0.1},            # temperate_forest
    7: {102: 0.3, 101: 0.2, 104: 0.4, 251: 0.02, None: 0.08},  # temperate_rainforest
    8: {102: 0.35, 104: 0.33, 101: 0.1, 109: 0.06, None: 0.16},  # boreal_forest
    9: {108: 0.06, None: 0.94},                                 # hell
}
...
original_tile_words = {None: "None",
                       ...
                       428: "Пух",
                       411: "Ведро с водой",
                       412: "Лечебная таблетка",
                       418: "Сильная таблетка",
                       413: "Зелье скорости",
                       414: "Зелье силы",
                       415: "Зелье каменной кожи",
                       416: "Зелье несгораемости",
                       417: "Зелье дыхания",
                       420: "Клык",
                       421: "Перо",
                       422: "Крыло мыши",
...
TILES_SOLIDITY = {
    ...
    103: 25,
    104: 5,
    # Новые растения: рвутся рукой, как трава. Гриб и мох чуть крепче — они
    # растут на камне, и «сорвать» их должно быть чуть дольше, чем травинку.
    113: 25,
    107: 8,
    108: 8,
    109: 8,
...
def item_of_right_click_tile(tile, res=True):
    ...
    if ttile == 101:  # куст с ягодами
        ...
        # Урожай снимается со ЗРЕЛОГО растения, и само оно остаётся: это и
        # делает грядку грядкой, а не одноразовой находкой.
        if tile[2] >= GROWING_PLANT_STAGES - 1:
            items = [(ttile, (1, 2), 1)]
    if res:
        res = [(i, cnt) for i, cnt, ch in items if ch == 1 or random.randint(0, 100 * 100) <= ch * 100 * 100]
        return res
