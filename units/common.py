...
last_versions = ["0.9.1", "0.1.3-alpha", "0.1.5-alpha", "0.1.6-alpha", "0.1.7-alpha",
                 "0.2.16-alpha", "0.2.17-alpha", "0.2.18-alpha", "0.2.19-alpha", "0.2.20-alpha", "0.2.21-alpha", "0.2.22-alpha", "0.2.23-alpha", "0.2.24-alpha", "0.2.25-alpha", "0.2.26-alpha"]
...
# Отмель. Ближе к краю чаши глубина становится меньше тайла, и раньше озеро в
# этом месте просто обрывалось: полный блок воды — и сразу порода, вертикальной
# стеной. Кромку теперь заливаем неполными уровнями воды (см. water_frame), и
# берег читается как отмель, а не как борт бассейна.
LAKE_SHOAL_MIN = 0.3       # тоньше этой доли тайла воды уже не ставим
LAKE_SHOAL_SPAN = 3.0      # с этой глубины ряд зеркала уже полный
# С этого ряда ниже зеркала вода рисуется глубинным (тёмным) вариантом: у
# водоёма появляется видимая толща, а не однородная синяя заливка.
LAKE_DEEP_ROW = 3
...
# Полоса, в которой площадка годится под озеро. Считается из границ среднего
# мира, а не задаётся руками: искать площадку вне этой полосы бессмысленно —
# lake_tile_at всё равно откажется раскладывать воду, и клетка решётки молча
# останется пустой (именно так и было: зеркало на -768 при границе -650).
LAKE_TOP_MIN = TOP_MIDDLE_WORLD + LAKE_RIM_CLEAR + 1
LAKE_TOP_MAX = BOTTOM_MIDDLE_WORLD - LAKE_MAX_DEPTH - 1
...
# Плавание (docs/WATER.md) ==================================================
# До этого вода была проходимой пустотой: в неё падали с той же скоростью, что
# и в воздухе, и выходили из неё тем же прыжком. Плавание — это три числа, и
# каждое отвечает за свою половину ощущения:
#
# * тонуть медленно (WATER_SINK_SPEED против max_fall_speed) — вода СПАСАЕТ от
#   падения, и это её первая функция в вертикальном мире;
# * гребок вверх повторяется, пока держат клавишу (WATER_SWIM_SPEED) — из воды
#   можно выплыть, а не только выпрыгнуть;
# * в воде медленно (WATER_DRAG) — иначе водоём ничем не отличался бы от
#   воздуха, и плыть было бы то же самое, что идти.
WATER_DRAG = 0.55           # доля горизонтальной скорости в воде
WATER_GRAVITY = 0.3         # доля обычного притяжения в воде
WATER_SINK_SPEED = 0.06     # предел скорости погружения без гребка
WATER_SWIM_SPEED = 0.16     # гребок вверх (прыжок игрока — 0.275)
# Плавать можно только там, где воды хотя бы половина тайла: плёнка на дне
# (уровень 1) — это лужа, по ней ходят, а не плывут.
WATER_SWIM_LEVEL = 2
# Существа считают ход в пикселях за кадр (movement_vector), а игрок — в
# пикселях за миллисекунду (умножается на elapsed_time). Поэтому те же пределы
# им нужны в своих единицах, иначе «медленно тонуть» у существ означало бы
# скорость в шестнадцать раз выше.
WATER_SINK_PX = WATER_SINK_SPEED * (1000 / FPS)
WATER_FLOAT_PX = 1.2        # всплытие наземного существа, пикселей за кадр

# Дыхание под водой. До этого в воде можно было жить вечно, а зелье дыхания
# только ускоряло плавание — то есть было подпоркой, а не ответом на угрозу.
#
# Почему теперь можно: раньше утопления не делали намеренно, потому что смерть
# от невидимого таймера — плохая цена за купание. Возражение снимается
# индикатором: полоска воздуха рисуется рядом со здоровьем, и игрок видит, что
# у него кончается, ещё до первого урона.
# Захлебнуться можно только там, где воды почти полный тайл: по колено и по
# пояс — это брод, там дышат. Порог выше, чем у плавания (WATER_SWIM_LEVEL),
# и в этой разнице весь смысл уровней воды для физики.
AIR_SUBMERGE_LEVEL = 3
AIR_MAX = FPS * 20          # сколько держится дыхание (в тактах)
AIR_REFILL_RATE = 8         # во сколько раз быстрее восстанавливается наверху
AIR_LOSS_DAMAGE = 3         # урон за секунду без воздуха
AIR_LOSS_PERIOD = FPS       # как часто он приходит
